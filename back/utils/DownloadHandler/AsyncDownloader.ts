...
export default class AsyncDownloader {
    ...
    private async queryOne(url: string, retrial: number = 0) {
        if (retrial)
        ...
        axios.get(url, {
            ...
            if (retrial < ENV.SETTINGS.MAX_RETRIAL)
                await this.queryOne(url, ++retrial);
            else 
                (new Logger(`HEAD: Max retrial achieved for ${url}`, SigLevel.error)).log();
        });
    }

    private async downloadPart(url: string, start: number, end: number, retrial: number = 0) {
        if (retrial)
        ...
        axios.get(url, {
            ...
            axiosErrorLogger(error, url);
            if (retrial < ENV.SETTINGS.MAX_RETRIAL)
                await this.downloadPart(url, start, end, ++retrial);
            else 
